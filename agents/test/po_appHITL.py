...
def purchaseorder_approval(
    ...
    with eng.connect() as conn:
        ...
        return {"message": f"Order placed, inventory updated, and PO generated. PO Number: {po_number}", "__end__": True}
...
def agnt_agent(req: ChatRequest):
    ...
    print(f"[agnt_agent] thread_id={thread_id} question='{question}'")
    if not question:
...
if __name__ == "__main__":
    uvicorn.run("po_appHITL:app", host="0.0.0.0", port=8000, reload=True)
